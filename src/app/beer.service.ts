...
export class BeerService {
  ...
  {
    return this.http.get<Beer[]>('https://beers-c344f.firebaseio.com/beers.json')
      .pipe(
        tap( data => {
          data
        }),
  ...
  getBeerByKey(key: string): Observable<Beer[]>{
    console.log('https://beers-c344f.firebaseio.com/beers/'+key+'.json')
  ...
  addBeer(beer: Beer): Observable<Beer> {
    let url = `https://beers-c344f.firebaseio.com/beers.json`;
  ...
  editBeer(beer: Beer, key: string): Observable<Beer> {
    const url = `https://beers-c344f.firebaseio.com/beers/`+key+'.json';
  ...
  deleteBeer(key: string): Observable<Beer>{
    let url = `https://beers-c344f.firebaseio.com/beers/`+key+'.json';
